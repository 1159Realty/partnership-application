...
import { ICampaignRecipient } from "@/lib/api/campaign/types";
...
interface Props {
  onRowClick?: (user: ICampaignRecipient) => void;
  data: PaginatedResponse<ICampaignRecipient> | null;
  page: number;
  limit: number;
...
function CampaignRecipientsTable({ data, page, limit, onLimitChange, onPageChange, onRowClick }: Props) {
  const rows =
    data?.items?.map((x) =>
      createTableData(
...
export { CampaignRecipientsTable };
