...
interface Column {
  id: "status" | "amount" | "dueDate" | "client" | "agent";
  label: string;
  minWidth?: number;
  align?: "right";
...
function createData(status: React.ReactNode, amount: string, dueDate: string, client: string, agent: string): Data {
  return { status, amount, dueDate, client, agent };
}

interface InvoiceTableProps {
  onRowClick?: (invoice: IInvoice) => void;
  data: PaginatedResponse<IInvoice> | null;
  page: number;
  limit: number;
...
function ModeratorInvoiceTable({ onRowClick, data, page, limit, onLimitChange, onPageChange }: InvoiceTableProps) {
  ...
  const rows =
    data?.items?.map((i) =>
      createData(
        ...
        getUserName(i?.enrolment?.client),
        getUserName(i?.enrolment?.agent)
      )
    ) || [];
