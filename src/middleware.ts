...
export default async function middleware(request: NextRequest) {
  ...
  if ((isProtectedRoute(currentPath) || isHomeRoute(request.nextUrl.pathname)) && !isSignedIn) {
    return NextResponse.redirect(new URL(`${ROUTES["/sign-in"]}?redirect=${encodeURIComponent(fullPath)}`, request.url));
  }
  ...
  if (isUnprotectedRoute(currentPath) && !isHomeRoute(request.nextUrl.pathname) && isSignedIn) {
  ...
  const isTryingDisallowedRoute =
    isProtectedRoute(currentPath) && allowedRoleRoutes.every((r) => !currentPath.includes(WEB_APP_URL + r));

  if (isTryingDisallowedRoute && !isHomeRoute(request.nextUrl.pathname) && isSignedIn) {
    return NextResponse.redirect(new URL(ROUTES["/"], request.url));
  }
}
