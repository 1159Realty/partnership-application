...
function EnrolledDetail({ enrollment, handleClose, onAddPlotId, handleCancel, handleResume }: InterestDetailProp) {
  ...
  const [plotId, setPlotId] = useState(enrollment?.plotId || "");
  ...
  useEffect(() => {
    if (enrollment?.plotId) {
      setPlotId(enrollment.plotId);
    }
  }, [enrollment]);
      ...
      <Box pb="48px" mt="32px">
        <Stack spacing={"32px"}>
          <Box px="16px">
            ...
            {hasPermission(userData?.roleId, "update-plot-id:enrollment") && (
              <Box mt="15px">
                {addPlotIdActive ? (
                  <Stack spacing={"10px"}>
                    <Box>
            ...
            {enrollment?.status === "COMPLETED" && (
              <Box
                ...
                mt="24px"
              >
                This enrollment has been completed.
              </Box>
            )}

            {hasPermission(userData?.roleId, "cancel:enrollment") && isOngoing && (
              <Box mt="24px">
                <Button
                  onClick={() => {
